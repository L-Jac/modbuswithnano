...
def motion_detect(src, area_thres, slope_thres, num_thres, relevance, relevance_time, noise_proportion, show_frame,
                  show_log, output, skip_time):
    """
    :param src: 待检测视频路径
    ...
    while cap.isOpened():
        ...
        frame_count += 1
        print(frame_count)
        log = 'Frame:' + str(frame_count) + '  noise_thres:' + str(noise_thres) + '  noise_proportion:' + str(
            noise_proportion)
        logs.append(log)
...
def main():
    ...
    parser.add_argument('--show_frame', type=bool, default=1)
    parser.add_argument('--show_log', type=bool, default=1)
